  ...
  return (
    <div className="animate-in fade-in duration-700 w-full pb-12">
      ...
      <div className="mb-6 flex justify-between items-end">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-zinc-100">Welcome back, {session?.user?.name || 'User'}</h1>
          <p className="text-zinc-500 mt-1">Here is your career pulse for today.</p>
        </div>
        <div className="text-right">
           <p className="text-[10px] text-zinc-400 font-bold uppercase tracking-widest mb-1">Total Activities</p>
           <p className="text-2xl font-bold text-indigo-400">{activities?.length || 0}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
        {statsItems.map((stat) => (
          <div key={stat.label} className="bg-zinc-900 border border-zinc-800 p-6 rounded-3xl group hover:shadow-2xl transition-all duration-300">
            ...
            <p className="text-3xl font-black text-zinc-50">{stat.value}</p>
          </div>
        ))}
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 auto-rows-[160px]">
        ...
        {/* Main Card (2x2) - Resume Strength */}
        <div className="md:col-span-2 md:row-span-2 group relative overflow-hidden bg-zinc-900 backdrop-blur-md border border-zinc-800 rounded-3xl p-6 md:p-8 flex flex-col justify-between hover:border-indigo-500 transition-all shadow-xl">
          <div className="absolute top-0 right-0 -mr-8 -mt-8 w-64 h-64 bg-indigo-500/10 rounded-full blur-3xl group-hover:bg-indigo-500/20 transition-all duration-500"></div>
          ...
          <div className="flex justify-between items-start relative z-10 w-full">
            <div className="flex-1">
              ...
              <h2 className="text-3xl font-bold text-zinc-50 mt-2">
            ...
            <div className="relative w-28 h-28 flex items-center justify-center shrink-0">
              ...
              <div className="absolute flex flex-col items-center">
                <span className="text-2xl font-black text-white">{stats?.score || 0}</span>
          ...
          <div className="relative z-10 mt-6 flex items-center justify-between">
            <Link href="/resume" className="inline-flex items-center gap-2 bg-indigo-500/10 text-indigo-400 hover:bg-indigo-500 hover:text-white px-5 py-2.5 rounded-xl font-bold text-xs uppercase tracking-widest transition-all group/btn border border-indigo-500/20">
            ...
            {stats?.filename && (
              <span className="text-[10px] text-zinc-500 font-medium max-w-[120px] truncate text-right">
        ...
        {/* Heat Map Card (2x1) */}
        <div className="md:col-span-2 md:row-span-1 bg-zinc-900/40 backdrop-blur-md border border-zinc-800/80 rounded-3xl p-5 hover:border-zinc-700/80 transition-all flex flex-col group relative overflow-hidden">
              ...
              <div className="grid grid-flow-col grid-rows-7 gap-1">
                 {heatData.slice(-140).map((day) => (
                    <div 
                      ...
                      className={`w-2.5 h-2.5 rounded-sm ${getColor(day.count)} transition-all hover:scale-125 cursor-help`}
                    ></div>
                 ))}
              </div>
           </div>
        ...
        {/* Job Card (1x1) - Compact Matches */}
        <div className="md:col-span-1 md:row-span-1 bg-zinc-900/40 backdrop-blur-md border border-zinc-800/80 rounded-3xl p-5 hover:border-zinc-700/80 transition-all flex flex-col group relative overflow-hidden">
           <div className="flex items-center justify-between mb-2 mt-1 relative z-10">
              <div className="flex items-center gap-2 text-emerald-400 font-medium text-sm">
                 <Briefcase className="h-4 w-4" />
                 <span>Jobs</span>
              </div>
              <Link href="/job-search" className="text-[10px] text-zinc-500 hover:text-zinc-300">View <ArrowUpRight className="h-2 w-2 inline" /></Link>
           </div>
        ...
        {/* Summarizer Card (1x1) - YT Summaries */}
        <div className="md:col-span-1 md:row-span-1 bg-zinc-900/40 backdrop-blur-md border border-zinc-800/80 rounded-3xl p-5 hover:border-zinc-700/80 transition-all flex flex-col group relative overflow-hidden">
           <div className="flex items-center justify-between mb-2 mt-1 relative z-10">
              ...
              <Link href="/summarizer" className="text-[10px] text-zinc-500 hover:text-zinc-300">View <ArrowUpRight className="h-2 w-2 inline" /></Link>
           </div>
        ...
        {/* News Ticker (1x1) */}
        <div className="md:col-span-1 md:row-span-1 bg-zinc-900/40 backdrop-blur-md border border-zinc-800/80 rounded-3xl p-5 hover:border-zinc-700/80 transition-all flex flex-col justify-between group">
           <div className="flex items-center gap-2 text-zinc-400 font-medium text-sm">
              <Rss className="h-4 w-4" /> News
           </div>
           
           <p className="text-[11px] text-zinc-200 leading-snug font-medium line-clamp-3">
